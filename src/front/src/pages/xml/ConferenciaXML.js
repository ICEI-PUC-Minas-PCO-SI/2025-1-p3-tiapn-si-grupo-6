...
import { editarProduto } from "../../api/produtos";
...
const ConferenciaXML = () => {
    ...
    const confirmarEntrada = async () => {
        ...
        try {
            for (let i = 0; i < itensPedido.length; i++) {
                const itemPedido = itensPedido[i];
                const itemXml = itensXml[i];

                if (!compararItens(itemXml, itemPedido)) continue;

                const produtoOriginal = itemPedido.produto;
                const novaQuantidade = produtoOriginal.quantidade + itemXml.quantidade;                
                ...
                await editarProduto(produtoOriginal.id, produtoAtualizado);
            }
    ...
    const todosIguais = () => {
        if (itensXml.length !== itensPedido.length) return false;

        return itensXml.every((itemXml, index) => {
            const itemPedido = itensPedido[index];
            return itemXml && itemPedido && compararItens(itemXml, itemPedido);
        });
    };
        ...
        <Box sx={{ p: 4 }}>
            ...
            <Grid container spacing={2} mb={3}>
                ...
                <Grid item>
                    <Button
                        variant="contained"
                        sx={{ backgroundColor: '#6039B8', color: 'white' }}
                        onClick={buscarItensDoPedido}
                    >
                        Buscar Pedido
                    </Button>
                </Grid>
